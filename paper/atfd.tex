...
LLM-judge consensus; (5)~a comparative evaluation of 6 systems---a
...
judge, and LangSmith with custom evaluator rules---revealing a stark
configuration effort gap; and
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Comparative evaluation.} An evaluation of 6 systems
    ...
    LLM judge, and LangSmith with custom evaluator rules---measured on
    detection rate, false positive rate, F1, category alignment, cost, and
    configuration effort. Claude Sonnet~4 achieves the highest F1 on real
    trajectories (0.800 on retail) and the highest category alignment (0.489)
    of any evaluated system. LangSmith achieves only 22\% DR on synthetic
    trajectories with 4 manually authored rules, illustrating the cost of
    rule-based platform configuration. Evaluation of additional commercial
    platforms (Braintrust) is planned as future work.
...
We evaluate 6 systems in this release and describe 2 additional
...
Two additional systems are planned for future evaluation:
GPT-4.1 judge (requiring paid OpenAI API access) and
Braintrust (requiring platform account and custom scorer configuration).
...
\begin{table}[t]
\centering
\caption{Main results: per-dataset breakdown for all 6 evaluated systems.
...
LangSmith dollar cost is \$0.00 (rule execution), but incurs 4 manually
authored rules and $\sim$15 min of setup effort not captured in \$/traj.}
\label{tab:cost}
\small
...
explicit across the three evaluation paradigms represented in this study.

\begin{table}[h]
\centering
\caption{Configuration effort by evaluation paradigm. LLM judges and the
Galea heuristic require no per-deployment rule authorship; LangSmith's
coverage is bounded by the rules written.}
...
degradation as a first-class failure mode. Our evaluation of 6
systems---a naive heuristic baseline, the Galea zero-config structural
heuristic, two open-weight LLM judges, Claude Sonnet~4, and LangSmith
with custom evaluator rules---on 1,162 trajectories from \taubench and
synthetic sources reveals a striking \emph{complementarity} in detection
profiles and an equally stark \emph{configuration effort gap}.
...
LangSmith, evaluated with 4 manually authored rules ($\sim$80 LOC,
$\sim$15 min setup), achieves only 22\% DR on synthetic trajectories---catching
tool loops and a coarse event-count threshold while missing every semantic
failure category (hallucination, safety violations, delegation failures). This
result quantifies the configuration effort gap: rule-driven platforms require
explicit authorship per failure category, making coverage a function of
engineering investment rather than a property of the tool itself.

These findings establish three conclusions about agent failure detection.
...
rule-driven platforms (LangSmith) scale coverage only with continued
engineering investment.

\paragraph{Future work.}
This initial release establishes the benchmark framework and baseline
results. Immediate next steps include: (1)~evaluating additional proprietary LLM
judges (GPT-4.1) and commercial platforms (Braintrust) to complete the
planned system comparison, and evaluating LangSmith with a more comprehensive
rule set (covering all 7 failure categories) to separate the effect of
platform capability from configuration effort;
