...
\swebench emerges as the ``easy'' domain: its failures are structurally
obvious (tool errors, test failures) and detectable even by the naive baseline
at 100\% DR, contrasting sharply with \taubench's semantically subtle failures.
Claude Sonnet~4 bridges the gap on hard domains, achieving {100\%} detection on both
...
\multirow{5}{*}{Naive Heuristic}
  ...
  & \swebench   & 100 & \textbf{100.0} {\scriptsize [92.9, 100.0]} & \textbf{0.0} {\scriptsize [0.0, 7.1]} & 0.667 & 0.000 \\
\midrule
\multirow{4}{*}{Galea Heuristic}
  ...
  & \swebench   & 100 & 90.0 {\scriptsize [78.6, 95.7]}            & \textbf{0.0} {\scriptsize [0.0, 7.1]}      & 0.643 & 0.000 \\
\midrule
...
\multirow{3}{*}{Claude Sonnet 4}
  ...
  & \swebench   & 10  & \textbf{100.0} {\scriptsize [61.0, 100.0]} & 0.0                                     & 0.000\textsuperscript{\S} & --- \\
\midrule
LangSmith
...
\bottomrule
\multicolumn{7}{@{}l}{\footnotesize \textsuperscript{\S}F1 = 0.000 for Claude Sonnet~4 on \swebench because all 10 evaluated trajectories are \texttt{pass} (no failures in the subset).}
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=2em]
  \item The naive heuristic achieves its highest DR on airline (40.9\%) and
    \swebench (100.0\%), where failures more frequently involve explicit tool
    errors and state violations that produce detectable error strings.
  ...
  \item \swebench is the ``easy'' domain: even the naive heuristic achieves
    100\% DR with 0\% FPR because failed coding trajectories contain explicit
    bash and test-suite errors. See \S\ref{sec:analysis:swebench} for
    detailed discussion.
  \item The Galea heuristic achieves 100\% DR on both retail and airline
    ...
    what structural pattern matching can provide. Notably, Galea's
    0\% FPR is a metric artifact: Galea emits only \texttt{warning}-severity
    findings, and FPR counts only \texttt{error}-severity findings on passing
    trajectories (see \S\ref{sec:analysis:swebench}).
...
100\% DR [92.9\%, 100.0\%] with 0\% FPR on \swebench, compared to only
25.4\% DR on the semantically subtle \taubench retail domain.

The Galea heuristic achieves 90.0\% DR [78.6\%, 95.7\%] on \swebench, slightly
below the naive heuristic's perfect score. This inversion---where Galea
...
A 10-trajectory pilot with Claude Sonnet~4 on \swebench achieves
100\% DR [61.0\%, 100.0\%] with 0\% FPR. The F1 of 0.000 reported in
Table~\ref{tab:main_results} reflects the evaluation subset composition
(all 10 trajectories were passing), not a genuine F1 failure; the metric
is undefined without failures in the evaluated subset.

\paragraph{Domain difficulty spectrum.}
Taken together, the three domains form a difficulty spectrum for monitoring tools:
\begin{itemize}[leftmargin=2em]
  \item \textbf{\swebench (easy)}: structural failures produce explicit error
    signals. All evaluated systems achieve high DR. The distinguishing
    challenge is not detection but \emph{attribution} (identifying which
    step caused the failure).
  \item \textbf{Synthetic (moderate)}: injected failures span structural and
    semantic categories. LLM judges excel; pure heuristics plateau at 78\%.
  \item \textbf{\taubench (hard)}: failures involve subtle argument errors,
    near-correct state transitions, and missing actions with no explicit
    error signal. Only the Galea heuristic and Claude Sonnet~4 achieve high
    DR; open-weight LLM judges collapse to 25\%.
\end{itemize}

This spectrum has a practical implication: benchmarking monitoring tools on
\swebench alone would dramatically overestimate production performance.
The \taubench and synthetic domains are essential for measuring the
...
1,312 trajectories from \taubench, \swebench, and synthetic sources reveals a
striking \emph{complementarity} in detection profiles, an equally stark
\emph{configuration effort gap}, a novel \emph{domain calibration}
failure mode, and a clear \emph{domain difficulty spectrum} ranging from
structurally obvious (\swebench) to semantically subtle (\taubench).
...
Fifth, domain difficulty varies substantially: \swebench is the easiest domain
(all systems achieve near-perfect DR on structural coding failures), while
\taubench is the hardest (open-weight judges collapse to 25\% DR on subtle
customer-service failures). Benchmarking monitoring tools on \swebench alone
would severely overestimate production performance in customer-service and
reasoning-intensive workflows. Monitoring tools should be evaluated across
the full difficulty spectrum to accurately characterize their operational
utility.

\paragraph{Future work.}
This initial release establishes the benchmark framework and baseline
results across three domains. Immediate next steps include:
(1)~evaluating GPT-4.1 as an additional proprietary LLM judge to complete
the planned system comparison, and evaluating LangSmith and Braintrust each
with a more comprehensive rule set (covering all 7 failure categories) to
measure the engineering investment required to close the coverage gap;
...
(3)~completing full LLM judge evaluation on the \swebench 100-trajectory
slice beyond the 10-trajectory pilot;
