...
LLM-judge consensus; (5)~an initial comparative evaluation of 3 systems---a
naive heuristic baseline and two open-weight LLM judges---with commercial
platform evaluation as future work; and
(6)~an open benchmark harness with cost-aware metrics. Our evaluation reveals
a stark \emph{detection gap}: LLM judges achieve {100\%} detection rate on
synthetic failures with obvious structural patterns but only {25.0\%} on
real \taubench failures involving subtle argument and state errors,
confirming that naturalistic agent failures remain
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Comparative evaluation.} An initial evaluation of 3 systems
    (\S\ref{sec:systems}--\S\ref{sec:results})---a naive heuristic baseline
    and two open-weight LLM judges (Llama~3.3~70B, Llama~4~Scout)---measured on
...
We evaluate 3 systems in this initial release and describe 5 additional
...
Galea$^\dagger$       & Hybrid    & A & 3 & Requires running Galea API \\
\bottomrule
\end{tabular}
\end{table}
...
Five additional systems are planned for future evaluation:
GPT-4.1 and Claude Sonnet~4 judges (requiring paid API access),
LangSmith and Braintrust (requiring platform accounts and custom evaluator
configuration), and Galea (requiring the Galea API to be running).
...
three systems evaluated in this initial release.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Both LLM judges achieve 100\% DR on synthetic data but struggle
    ...
    on retail, comparable to the naive heuristic (25.4\%).
  \item Category alignment is near zero for all systems on real data,
    ...
    \emph{type} is almost never correctly identified.
\end{itemize}
...
throughput. Tokens/traj = mean tokens consumed per trajectory.}
\label{tab:cost}
\small
\begin{tabular}{@{}lrrrrr@{}}
\toprule
...
Synthetic failures with obvious structural patterns---\texttt{tool\_loop},
\texttt{infinite\_delegation}, \texttt{context\_overflow}---are detected
at 100\% by LLM judges. The naive heuristic detects these at only 14\%,
suggesting that even ``easy'' structural patterns require semantic
understanding when they lack explicit error strings.
...
degradation as a first-class failure mode. Our initial evaluation of 3
systems---a naive heuristic baseline and two open-weight LLM judges---on
1,162 trajectories from \taubench and synthetic sources reveals a stark
\emph{detection gap}: LLM judges achieve 100\% detection on synthetic
failures with obvious structural patterns but only 25\% on real
\taubench failures involving subtle argument and state errors. Category
alignment is near zero on real data, indicating that correctly identifying
failure \emph{type} is dramatically harder than detecting failure
\emph{existence}. The naive heuristic baseline performs surprisingly well
on airline data (40.9\% DR) where tool errors produce explicit error
strings, but achieves only 8.3\% on telecom where failures are
predominantly subtle.

These findings establish that agent failure detection is a genuinely
difficult problem: even on the easiest subset (synthetic failures),
category alignment peaks at 0.359, and on real data, the best
systems perform comparably to naive pattern matching.
...
Braintrust, Galea) to complete the planned 8-system comparison;
(2)~acquiring and integrating \swebench trajectories;
(3)~running the multi-judge consensus pipeline for ground truth
validation; (4)~applying domain-specific quality rubrics to enable the
\texttt{degraded} tier and QDR measurement; (5)~expanding domain coverage
to medical, legal, and financial workflows; (6)~adding streaming
evaluation for real-time monitoring tools; and (7)~investigating why
category alignment is so low and whether alternative prompting strategies
or fine-tuned models can improve failure type identification.
