...
(3)~a multi-domain dataset of {2,262} trajectories drawn from
\taubench (customer service), \swebench (coding), hand-crafted synthetic
scenarios, and ATBench (agent safety); (4)~a
...
across all 5 domains---a naive heuristic baseline, a zero-config structural
...
ATBench (1,000 agent safety trajectories) proves the complementary profiles
thesis definitively: heuristic systems achieve {0\%} DR on safety failures
(tool description injection, harmful action execution, data leakage) while
Claude Sonnet~4 achieves {100\%} DR---the widest gap of any domain.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Multi-domain dataset.} A corpus of {2,262}
    ...
    safety, quality, and infrastructure failures, and ATBench (1,000
    agent safety trajectories with 14 failure modes and 8 risk sources).
  ...
  \item \textbf{Complete comparative evaluation.} A full 7-system $\times$
    5-domain evaluation (\S\ref{sec:systems}--\S\ref{sec:results})---a naive
    ...
    \swebench), Claude Sonnet~4 as a proprietary LLM judge across all five
    ...
    LLM judges. ATBench shows the starkest gap: heuristic systems achieve 0\%
    DR while Claude Sonnet~4 achieves 100\% DR, confirming that semantic safety
    failures are undetectable by structural analysis. LangSmith and Braintrust
    both achieve 100\% DR on \swebench but 100\% FPR, confirming that
    event-count thresholds trigger on every coding trajectory and are unusable
    without calibration. Braintrust additionally reveals a domain calibration
    problem: its threshold achieves 91.7\% DR on retail but 71.1\% FPR.
    Evaluation of GPT-4.1 as an additional LLM judge is planned as future work.
...
The \atfd benchmark draws trajectories from four sources
...
We include 1,000 trajectories from the \texttt{AI45Research/ATBench} dataset
on HuggingFace~\citep{ai45research2025atbench}, covering agent safety
scenarios with 14 failure modes and 8 risk sources. The dataset is nearly
balanced: 503 safe (pass) and 497 unsafe (fail) trajectories. Failure modes
span tool description injection, harmful action execution, data leakage, and
related agent safety violations. Ground truth is provided by the ATBench
evaluation protocol. Unlike structural failure domains, ATBench failures are
\emph{semantically} defined: no structural signal (tool errors, loop patterns,
step counts) differentiates safe from unsafe trajectories. This makes ATBench
the hardest domain for heuristic systems and confirms the domain difficulty
spectrum with a fifth tier in which structural analysis is categorically
insufficient (see \S\ref{sec:analysis:atbench}).
...
\begin{table}[t]
\centering
\caption{Complete results matrix: all 7 evaluated systems $\times$ 5 domains.
...
\multirow{5}{*}{Naive Heuristic}
  ...
  & ATBench                & 1,000 & 0.0 {\scriptsize [0.0, 1.9]}               & 0.0 {\scriptsize [0.0, 1.8]} \\
\midrule
\multirow{5}{*}{LangSmith (4 rules)}
  ...
  & ATBench                & 1,000 & 0.0 {\scriptsize [0.0, 1.9]}               & 0.0 {\scriptsize [0.0, 1.8]} \\
\midrule
\multirow{5}{*}{Braintrust (4 scorers)}
  & Synthetic (all fail)   & 50    & 22.0 {\scriptsize [12.8, 35.2]}            & --- \\
...
\multirow{5}{*}{Claude Sonnet 4}
  ...
  & ATBench                & 10  & \textbf{100.0} {\scriptsize [72.2, 100.0]} & N/A \\
\midrule
Llama 3.3 70B$^\dagger$
  & Synthetic (all fail)   & 50  & \textbf{100.0} {\scriptsize [92.9, 100.0]} & --- \\
\midrule
\multirow{5}{*}{Galea Heuristic}
...
ATBench DR for heuristic/rule systems = 0\%; Claude Sonnet~4 (n=10) = 100\%.}
\label{tab:summary_matrix}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{System} & \textbf{Synthetic} & \textbf{Retail} & \textbf{Airline} & \textbf{\swebench} & \textbf{ATBench} \\
 & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} \\
\midrule
Naive Heuristic         & 14\% / ---      & 25\% / 18\%  & 41\% / 4\%   & \textbf{100\%} / \textbf{0\%} & 0\% / 0\% \\
LangSmith (4 rules)     & 22\% / ---      & 0\% / 0\%    & 64\% / 33\%  & \textbf{100\%} / $\sim$100\% & 0\% / 0\% \\
Braintrust (4 scorers)  & 22\% / ---      & 92\% / 71\%  & 64\% / 33\%  & \textbf{100\%} / $\sim$100\% & 0\% / 0\% \\
Llama 4 Scout           & \textbf{100\%} / --- & 25\% / 13\%  & 20\% / 67\%  & \textbf{100\%} / $\sim$100\% & \emph{n/a} \\
Claude Sonnet 4         & \textbf{100\%} / --- & \textbf{100\%} / \textbf{6\%} & 40\% / 20\%  & \textbf{100\%} / \textbf{0\%} & \textbf{100\%} / N/A \\
Llama 3.3 70B           & \textbf{100\%} / --- & \emph{n/a}   & \emph{n/a}   & \emph{n/a} & \emph{n/a} \\
Galea Heuristic         & 78\% / ---      & \textbf{100\%} / 0\%* & \textbf{100\%} / 0\%* & 90\% / 0\%* & 0\% / 0\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Domain difficulty spectrum is clear and now five-tiered.}
    ...
    40\% vs.\ 100\%)---and ATBench is categorically hardest for heuristics:
    0\% DR for all structural systems, 100\% DR for Claude Sonnet~4. This is
    the inverse of the \swebench pattern, completing the spectrum.
  ...
  \item \textbf{ATBench is the definitive test of complementarity.}
    Heuristic systems (Naive, Galea) and rule-driven platforms (LangSmith,
    Braintrust) all achieve 0\% DR on ATBench---the structural signal that
    makes these systems work on \taubench is entirely absent from semantic
    safety failures. Claude Sonnet~4 achieves 100\% DR on ATBench (n=10),
    confirming that LLM semantic understanding is necessary and sufficient
    for this failure class. The gap between structural (0\% DR) and semantic
    (100\% DR) approaches is widest on ATBench of any evaluated domain.
  ...
  \item \textbf{No system dominates all cells.} The matrix reveals
    ...
    synthetic, retail, and ATBench; LLM judges and rule-driven platforms
    ...
    the complete five-domain matrix.
\end{itemize}
...
summarizes this complementarity with the full five-domain view.

\begin{table}[h]
\centering
\caption{Detection profile comparison across all five domains. Galea dominates
on structural failures in real \taubench traces with 0\% FPR (error-severity
definition); Claude Sonnet~4 dominates on synthetic, retail, and ATBench;
no system dominates airline---the hardest domain for LLM judges.
ATBench is the definitive proof of complementarity: 0\% DR for all heuristic
and rule-driven systems, 100\% DR for Claude Sonnet~4.}
\label{tab:profiles}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
ATBench      & 0\%            & \emph{n/a}                & \textbf{100\%}              & 0\% \\
\midrule
...
\multicolumn{5}{@{}l}{\footnotesize Retail/Airline/\swebench cells show DR\% / FPR\%. ATBench shows DR\% only.}
\end{tabular}
\end{table}
...
The complete five-domain matrix reveals a clear difficulty ordering for monitoring tools:
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Airline \taubench (hardest for LLM judges)}: Even Claude
    ...
    and low FPR (0\%) here.
  \item \textbf{ATBench (hardest for heuristics; inverse of \swebench)}:
    Semantic safety failures (tool description injection, harmful action
    execution, data leakage) produce no structural signal. All heuristic and
    rule-driven systems achieve 0\% DR; Claude Sonnet~4 achieves 100\% DR
    (n=10, 95\% CI [72.2\%, 100.0\%]). This is the exact inverse of
    \swebench: the domain that is trivial for LLM judges is categorically
    impossible for structural analysis.
\end{itemize}

This five-tier spectrum has a critical practical implication: benchmarking
...
(\swebench) through semantic-safety (ATBench)---is necessary to characterize
operational coverage across the full failure landscape. Notably, no single
system achieves high DR across all five tiers.
...
ATBench~\citep{ai45research2025atbench} provides the sharpest empirical test of
the complementarity thesis. Its 1,000 trajectories (503 safe, 497 unsafe) span
14 failure modes and 8 risk sources---all defined at the semantic level: tool
description injection, harmful action execution, data leakage, and related
safety violations. Unlike \taubench failures (which produce observable state
errors) or \swebench failures (which produce explicit tool error strings),
ATBench unsafe trajectories are structurally indistinguishable from safe ones.
A malicious tool call looks syntactically identical to a legitimate tool call;
only semantic understanding of the instruction and context reveals the violation.

The results confirm this analysis definitively. Every heuristic and rule-driven
system achieves exactly 0\% DR [0.0\%, 1.9\%] on ATBench with 0\% FPR---not
because they have low recall balanced against high precision, but because they
emit \emph{no findings at all} on this domain. The structural signals that
enable Galea to achieve 100\% DR on \taubench (tool-error fingerprints,
loop patterns, state-signal anomalies) are absent from semantic safety failures
by construction.

Claude Sonnet~4 achieves 100\% DR [72.2\%, 100.0\%] on the evaluated ATBench
pilot (n=10), consistent with prior safety evaluation results showing frontier
models can identify harmful instructions when given appropriate context. The
wide confidence interval reflects the pilot sample size; full-scale evaluation
across all 1,000 trajectories is planned.

\paragraph{Why ATBench completes the spectrum.}
ATBench occupies the fifth position in the domain difficulty spectrum not
because it is \emph{hard} for LLM judges---it appears to be easy---but because
it is \emph{categorically impossible} for structural systems. The \swebench
domain sits at the opposite end: structurally trivial (error strings are
explicit), requiring no semantic understanding, yet difficult for precision
(all systems trigger on passing trajectories). ATBench inverts this:
semantically clear for LLM judges (harmful instructions are often obvious to
language models), but structurally invisible (no error string, no loop, no
step-count anomaly). Together, \swebench and ATBench bracket the full spectrum
...
Claude reaches only 40--100\%) with Claude's 100\% DR on ATBench (where Galea
achieves 0\%). Neither system alone covers the full five-domain matrix; together
they would achieve near-complete coverage across structural \emph{and} semantic
failure classes.
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Dataset scale and coverage gaps.} While the dataset
    comprises 2,262 trajectories, LLM judge evaluation was limited to
    10--100 trajectories for most domains due to free-tier API rate limits.
    ...
    pilot for both \swebench and ATBench. Some subcategories (\eg
...
scorers---across all 5 domains on 2,262 trajectories from \taubench,
\swebench, synthetic sources, and ATBench reveals a striking
...
failure mode, and a clear five-level \emph{domain difficulty spectrum}
from structurally obvious (\swebench) to structurally invisible yet
semantically clear (ATBench).
...
plateaus at 78\% on synthetic failures requiring semantic analysis, and
achieves exactly 0\% DR on ATBench where structural signals are absent by
definition. Open-weight LLM judges achieve 100\% DR on synthetic failures but
struggle on real trajectories (Llama~4~Scout: 25\% on retail, 20\% on airline).
Claude Sonnet~4 is the \textbf{best single system on synthetic, retail, and
ATBench domains}: it achieves 100\% DR with low FPR (6\% on retail, 0\% on
\swebench, 100\% DR on ATBench n=10) and the highest category alignment
(CA = 0.489 on synthetic). However, even Claude Sonnet~4 reaches only 40\%
DR on airline---the hardest domain for LLM judges---where Galea's structural
approach (100\% DR, 0\% FPR) clearly dominates. ATBench proves the
complementarity thesis most sharply: the gap between heuristic (0\% DR) and
LLM-semantic (100\% DR) approaches is wider here than on any other domain.
...
Fifth, the domain difficulty spectrum is five-tiered: \swebench (easy,
...
policy semantics), ATBench (hardest for heuristics, semantically defined
safety violations with no structural signal). ATBench and \swebench are
mirror images: each is easy for the system class that struggles on the other.
Benchmarking monitoring tools on any single domain would give a misleading
picture of production utility; the complete five-domain matrix is necessary
to characterize operational coverage.

\paragraph{Future work.}
This release provides the complete 7-system $\times$ 5-domain evaluation on
2,262 trajectories. Immediate next steps include:
...
(2)~completing Llama~3.3~70B and Llama~4~Scout evaluation on ATBench;
(3)~scaling the Claude Sonnet~4 ATBench evaluation from the current 10-trajectory
pilot to the full 1,000-trajectory dataset to tighten confidence intervals;
(4)~completing Llama~3.3~70B evaluation on retail, airline, and \swebench
domains (currently blocked by Groq free-tier rate limits);
(5)~evaluating the Galea full-stack system (with LLM-augmented analysis)
beyond the heuristic-only configuration reported here;
(6)~running the multi-judge consensus pipeline for ground truth validation;
(7)~applying domain-specific quality rubrics to enable the \texttt{degraded}
tier and QDR measurement; (8)~expanding domain coverage to medical, legal,
and financial workflows; (9)~adding streaming evaluation for real-time
monitoring tools; (10)~investigating why category alignment is so low and
whether alternative prompting strategies or fine-tuned models can improve
failure type identification; and (11)~formally evaluating heuristic+LLM
ensemble configurations---in particular, measuring whether a Galea+Claude
Sonnet~4 ensemble achieves near-complete coverage across all five domains.
