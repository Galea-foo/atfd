...
LLM-judge consensus; (5)~a comparative evaluation of 5 systems---a
naive heuristic baseline, a zero-config structural heuristic (Galea),
two open-weight LLM judges, and Claude Sonnet~4 as a proprietary LLM
judge---with commercial platform evaluation as future work; and
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Comparative evaluation.} An evaluation of 5 systems
    ...
    (Llama~3.3~70B, Llama~4~Scout), and Claude Sonnet~4 as a proprietary
    LLM judge---measured on detection rate, false positive rate, F1,
    category alignment, and cost. Claude Sonnet~4 achieves the highest
    F1 on real trajectories (0.800 on retail) and the highest category
    alignment (0.489) of any evaluated system. Evaluation of commercial
    platforms is planned as future work.
...
We evaluate 5 systems in this release and describe 3 additional
...
Three additional systems are planned for future evaluation:
...
throughput. Claude Sonnet~4 was accessed at \$0.00 via headless API access.
Tokens/traj = mean tokens consumed per trajectory. Galea
...
Claude Sonnet 4    & 0.000 & 0.00 & 13.4--47.7\textsuperscript{\textdaggerdbl}     & 254--1,546\textsuperscript{\textdaggerdbl} & 70 \\
\bottomrule
...
degradation as a first-class failure mode. Our evaluation of 5
systems---a naive heuristic baseline, the Galea zero-config structural
heuristic, two open-weight LLM judges, and Claude Sonnet~4---on 1,162
trajectories from \taubench and synthetic sources reveals a striking
\emph{complementarity} in detection profiles.
...
These findings establish two conclusions about agent failure detection.
...
judges (GPT-4.1) and commercial platforms (LangSmith,
