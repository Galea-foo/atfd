...
(3)~a multi-domain dataset of {2,462} trajectories spanning
\taubench (customer service), \swebench (coding), hand-crafted synthetic
scenarios, ATBench (agent safety, 200-sample subset), and Toolathlon
(long-horizon tool use); (4)~a
...
across all 6 domains---a naive heuristic baseline, a zero-config structural
...
ATBench (200-sample subset of 1,000 safety trajectories) reveals a middle
ground for Galea ({52.5\%} DR between its structural strengths and limits)
and full {100\%} DR for Claude Sonnet~4; Galea reaches {52.5\%} by detecting
structurally co-occurring anomalies, while the remaining {47.5\%} require
semantic understanding.
Toolathlon (100 long-horizon trajectories with 600+ tools, up to 16k messages)
is the hardest domain for \emph{all} systems except Claude Sonnet~4, which
uniquely achieves {100\%} DR even here, though at a high FPR of {67\%};
every other evaluated system achieves {0\%} DR, confirming that long-horizon
multi-step failures require frontier semantic understanding and cannot be caught
by structural analysis or rule-based approaches.
...
Claude Sonnet~4 is the \emph{only} system achieving non-zero DR across all
six domains, with the highest category alignment (CA = 0.489) of any evaluated
system, though FPR escalates from {0\%} on \swebench to {67\%} on Toolathlon.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Multi-domain dataset.} A corpus of {2,462}
    trajectories (\S\ref{sec:datasets}) spanning customer service
    (\taubench retail, airline, and telecom), coding (\swebench, 100
    OpenHands trajectories), hand-crafted synthetic scenarios covering
    safety, quality, and infrastructure failures, ATBench (200-sample
    subset of 1,000 agent safety trajectories with 14 failure modes
    and 8 risk sources), and Toolathlon (100 long-horizon multi-tool
    trajectories from HKUST-NLP, ICLR 2026).
  ...
  \item \textbf{Complete comparative evaluation.} A full 7-system $\times$
    ...
    \swebench), Claude Sonnet~4 as a proprietary LLM judge across all six
    ...
    LLM judges---and 67\% FPR on Toolathlon. ATBench shows the starkest
    heuristic-vs-LLM gap: heuristic systems achieve 0\% DR while Claude
    Sonnet~4 achieves 100\% DR. Toolathlon is the hardest domain for all
    systems except Claude Sonnet~4: every other system achieves 0\% DR on
    long-horizon multi-tool trajectories, confirming that multi-step plan
    failures require semantic understanding of tool sequencing that structural
    and rule-based approaches cannot provide. LangSmith and Braintrust both
    achieve 100\% DR on \swebench but 100\% FPR, confirming that event-count
    thresholds are unusable without calibration. Braintrust additionally
    reveals a domain calibration problem: its threshold achieves 91.7\% DR
    on retail but 71.1\% FPR.
...
The \atfd benchmark draws trajectories from five sources
...
We include a 200-sample subset from the \texttt{AI45Research/ATBench} dataset
on HuggingFace~\citep{ai45research2025atbench} (full dataset: 1,000
trajectories). ATBench covers agent safety scenarios with 14 failure modes
and 8 risk sources; the full dataset is nearly balanced (503 safe, 497
unsafe), and we sample proportionally (approximately 101 safe, 99 unsafe)
to preserve this balance. Failure modes span tool description injection,
harmful action execution, data leakage, and related agent safety violations.
Ground truth is provided by the ATBench evaluation protocol. Unlike
structural failure domains, ATBench failures are \emph{semantically} defined:
no structural signal (tool errors, loop patterns, step counts) differentiates
safe from unsafe trajectories. This makes ATBench the hardest domain for
heuristic systems and confirms a fifth tier of the domain difficulty spectrum
in which structural analysis is categorically insufficient
(see \S\ref{sec:analysis:atbench}).

\paragraph{Toolathlon trajectories.}
We include 100 trajectories from the
\texttt{hkust-nlp/Toolathlon-Trajectories} dataset on
HuggingFace~\citep{toolathlon2026}, generated by Claude~4~Sonnet on
long-horizon multi-tool tasks spanning 600+ available tools. Toolathlon
(HKUST-NLP, ICLR 2026) is distinguished by the scale and complexity of
its trajectories: individual runs may contain up to 16,000 messages across
hundreds of tool calls. We cap each trajectory at 100 events for evaluation
to bound cost and latency; the full trajectory is retained for statistical
analysis. Ground truth is derived from the \texttt{task\_status.evaluation}
field: trajectories labeled \texttt{true} (pass) or \texttt{false} (fail)
are included; \texttt{null} (inconclusive) is excluded. Unlike \taubench,
where failures involve argument or state errors, and unlike ATBench, where
failures are semantically clear harmful instructions, Toolathlon failures
involve \emph{multi-step plan failures}: the agent selects a coherent but
incorrect sequence of tools, reaches an intermediate dead end, or
misunderstands dependencies across a long context. This makes Toolathlon
the hardest domain for all evaluated systems except Claude Sonnet~4 and
establishes a sixth tier of the domain difficulty spectrum
(see \S\ref{sec:analysis:toolathlon}).
...
Llama~4~Scout was evaluated on all six domains (n=50 synthetic, n=50 retail,
n=20 airline, n=20 \swebench, n=200 ATBench, n=100 Toolathlon).
Llama~3.3~70B was evaluated only on synthetic (n=50): Groq free-tier rate
limits made all other domains infeasible within the evaluation window.

\paragraph{Claude Sonnet~4 Judge (evaluated).}
Claude Sonnet~4 was accessed via the Anthropic API and evaluated across all
six domains: synthetic (n=50), retail (n=20), airline (n=20), \swebench
(n=20), ATBench (n=200), and Toolathlon (n=100). It receives the same full
trajectory and taxonomy-structured system prompt as the Llama judges.
Claude Sonnet~4 is the only system achieving non-zero DR across all six
domains: 100\% DR on synthetic, retail, \swebench, ATBench, and Toolathlon,
with 40\% DR on airline. It delivers the highest category alignment
(CA = 0.489 on synthetic) of any evaluated system and is the only LLM judge
achieving both 100\% DR and 0\% FPR on \swebench. Its FPR escalates on
harder domains: 6\% on retail, 20\% on airline, and 67\% on Toolathlon---
the last reflecting calibration challenges in extremely long-context
...
results tables.
LangSmith and Braintrust are both fully evaluated across all six domains
...
seven evaluated systems across all six benchmark domains.
...
rate limits prevented all other domain runs (see \S\ref{sec:systems}).

\begin{table}[t]
\centering
\caption{Complete results matrix: all 7 evaluated systems $\times$ 6 domains.
...
other domains. ATBench FPR is N/A for systems with 0\% DR (no findings emitted).
...
\multirow{6}{*}{Naive Heuristic}
  ...
  & Toolathlon             & 100   & 0.0 {\scriptsize [0.0, 8.0]}               & 0.0 {\scriptsize [0.0, 11.4]} \\
\midrule
...
\multirow{6}{*}{Llama 4 Scout}
  ...
  & Toolathlon             & 100 & 14.3 {\scriptsize [7.1, 26.5]}             & \textbf{0.0} {\scriptsize [0.0, 11.4]} \\
\midrule
\multirow{6}{*}{Claude Sonnet 4}
  ...
  & Toolathlon             & 100 & \textbf{100.0} {\scriptsize [96.3, 100.0]} & 66.7 {\scriptsize [39.1, 86.2]} \\
\midrule
Llama 3.3 70B$^\dagger$
  & Synthetic (all fail)   & 50  & \textbf{100.0} {\scriptsize [92.9, 100.0]} & --- \\
\midrule
\multirow{6}{*}{Galea Heuristic}
  ...
  & ATBench (200)          & 200   & 52.5 {\scriptsize [45.5, 59.5]}            & \textbf{0.0*} {\scriptsize [0.0, 5.6]} \\
  & Toolathlon             & 100   & 0.0 {\scriptsize [0.0, 8.0]}               & 0.0 {\scriptsize [0.0, 11.4]} \\
\bottomrule
...
set or FPR not separately measured for this system/domain pair).
``$\sim$100'' = FPR $\approx$ 100\% (event-count threshold fires on every
passing trajectory). Galea FPR uses \texttt{error}-severity definition;
see note in Table~\ref{tab:main_results}.
Llama~3.3~70B evaluated on synthetic only.}
\label{tab:summary_matrix}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{System} & \textbf{Synthetic} & \textbf{Retail} & \textbf{Airline} & \textbf{\swebench} & \textbf{ATBench} & \textbf{Toolathlon} \\
 & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} & \textbf{DR / FPR} \\
\midrule
Naive Heuristic         & 14\% / ---      & 25\% / 18\%  & 41\% / 4\%   & \textbf{100\%} / \textbf{0\%} & 0\% / 0\%  & 0\% / 0\% \\
LangSmith (4 rules)     & 22\% / ---      & 0\% / 0\%    & 64\% / 33\%  & \textbf{100\%} / $\sim$100\% & 0\% / 0\%  & 0\% / 0\% \\
Braintrust (4 scorers)  & 22\% / ---      & 92\% / 71\%  & 64\% / 33\%  & \textbf{100\%} / $\sim$100\% & 0\% / 0\%  & 0\% / 0\% \\
Llama 4 Scout           & \textbf{100\%} / --- & 25\% / 13\%  & 20\% / 67\%  & \textbf{100\%} / $\sim$100\% & 70\% / --- & 14\% / \textbf{0\%} \\
Claude Sonnet 4         & \textbf{100\%} / --- & \textbf{100\%} / \textbf{6\%} & 40\% / 20\%  & \textbf{100\%} / \textbf{0\%} & \textbf{100\%} / --- & \textbf{100\%} / 67\% \\
Llama 3.3 70B           & \textbf{100\%} / --- & \emph{n/a}   & \emph{n/a}   & \emph{n/a} & \emph{n/a} & \emph{n/a} \\
Galea Heuristic         & 78\% / ---      & \textbf{100\%} / 0\%* & \textbf{100\%} / 0\%* & 90\% / 0\%* & 53\% / 0\%* & 0\% / 0\% \\
\bottomrule
\end{tabular}
\end{table}
...
enables seven cross-domain observations that are only visible once all cells are
populated.

\begin{itemize}[leftmargin=2em]
  \item \textbf{Domain difficulty spectrum is clear and now six-tiered.}
    \swebench is the easiest domain (all systems achieve high DR on structural
    coding errors), synthetic is moderate (LLM judges excel, pure heuristics
    plateau at 78\%), the \taubench domains are hard---with airline harder than
    retail for LLM judges (Llama~4~Scout: 20\% vs.\ 25\%; Claude Sonnet~4:
    40\% vs.\ 100\%)---ATBench reveals a middle ground for heuristics
    (Galea: 52.5\% DR, between its 0\% on Toolathlon and 100\% on \taubench),
    and Toolathlon is the hardest domain for all systems except Claude
    Sonnet~4. The spectrum: \swebench $\to$ Synthetic $\to$ Retail $\to$
    Airline $\to$ ATBench $\to$ Toolathlon, each posing qualitatively distinct
    challenges.
  ...
  \item \textbf{Toolathlon is hardest for all systems except Claude Sonnet~4.}
    Every system except Claude Sonnet~4 achieves exactly 0\% DR on Toolathlon:
    the naive heuristic, Galea, LangSmith, and Braintrust all emit no findings
    on long-horizon multi-tool trajectories; Llama~4~Scout reaches only 14.3\%
    DR with 0\% FPR. Only Claude Sonnet~4 achieves 100\% DR on Toolathlon,
    but at a high FPR (66.7\%)---the highest FPR of any Claude cell across all
    domains. Long-horizon failures require semantic understanding of multi-step
    plans; structural pattern matching and simple rules cannot detect whether
    a 16,000-message trajectory with hundreds of tool calls reached the correct
    goal.

  \item \textbf{ATBench reveals a middle ground for Galea.} Unlike prior
    domains where Galea achieves either 100\% DR (\taubench) or 0\% DR
    (Toolathlon), Galea achieves 52.5\% DR on ATBench with 0\% FPR
    (error-severity definition). This confirms that some safety failure
    patterns---tool-description anomalies and structural irregularities that
    co-occur with unsafe actions---are partially detectable by structural
    analysis, while the remaining 47.5\% require semantic understanding.

  \item \textbf{Claude Sonnet~4 is the only system with 100\% DR across
    all six domains}, though FPR escalates on harder domains (0\% on
    \swebench, 6\% on retail, 20\% on airline, 67\% on Toolathlon). Its
    FPR on retail (5.6\%) is substantially lower than Llama~4~Scout (13.2\%)
    and the naive heuristic (17.8\%). Its category alignment (CA = 0.489
    on synthetic) remains the highest of any evaluated system.
  ...
  \item \textbf{Galea is the only system with 0\% FPR across all domains}
    ...
    retail and airline with 0\% FPR, 90\% on \swebench, 52.5\% on ATBench,
    and 78\% on synthetic---in every case with 0\% FPR. Its 0\% DR on
    Toolathlon is the one cell where structural analysis is categorically
    insufficient.

  \item \textbf{No system dominates all cells.} The matrix reveals
    complementary strengths: Galea dominates on real \taubench domains (100\%
    DR, 0\% FPR on retail and airline); Claude Sonnet~4 is the only system
    achieving 100\% DR on all six domains; LLM judges and rule-driven platforms
    ...
    the complete six-domain matrix.
\end{itemize}
...
Galea Heuristic    & 0 rules, 0 min & 0.000 & 0.039--0.054\textsuperscript{\textdagger}     & 0 & 600 \\
...
summarizes this complementarity with the full six-domain view.

\begin{table}[h]
\centering
\caption{Detection profile comparison across all six domains. Galea dominates
on structural failures in real \taubench traces with 0\% FPR (error-severity
definition); Claude Sonnet~4 is the only system achieving non-zero DR across
all six domains; Toolathlon is the frontier where all systems except Claude
Sonnet~4 achieve 0\% DR; ATBench reveals a middle ground for Galea (52.5\%
DR) between its structural strengths and semantic limits.}
\label{tab:profiles}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Retail       & \textbf{100\%, 0\%*} & 25\%, 13\%            & \textbf{100\%, 6\%}         & 0--92\%, 0--71\% \\
Airline      & \textbf{100\%, 0\%*} & 20\%, 67\%            & 40\%, 20\%                  & 64\%, 33\% \\
\swebench    & 90\%, 0\%*           & \textbf{100\%}, 100\% & \textbf{100\%, 0\%}         & \textbf{100\%}, 100\% \\
ATBench      & 53\%, 0\%*           & 70\%                  & \textbf{100\%}              & 0\% \\
Toolathlon   & 0\%                  & 14\%, 0\%             & \textbf{100\%, 67\%}        & 0\% \\
\midrule
CA (synth.)  & 0.000                & 0.318--0.359          & \textbf{0.489}               & --- \\
\bottomrule
\multicolumn{5}{@{}l}{\footnotesize *Galea FPR = 0\% under error-severity definition; see \S\ref{sec:analysis:swebench}.} \\
\multicolumn{5}{@{}l}{\footnotesize Retail/Airline/\swebench/ATBench/Toolathlon cells show DR\% / FPR\%. Open-wt. = Llama~4~Scout.}
\end{tabular}
\end{table}
...
The complementarity across the complete six-domain matrix strengthens the
case for an ensemble architecture: run the Galea heuristic first (zero
latency, zero cost, zero FPR) to catch structural failures across all
domains, then invoke Claude Sonnet~4 for synthetic, retail-style, ATBench,
and Toolathlon trajectories where semantic understanding is needed. Neither
system alone covers the full matrix; together they would combine Galea's
100\% retail/airline coverage and 52.5\% ATBench coverage with Claude's
100\% synthetic/\swebench/ATBench/Toolathlon coverage and superior category
alignment---though Claude's 67\% FPR on Toolathlon remains an open challenge
for ensemble precision.
...
The complete six-domain matrix reveals a clear difficulty ordering for monitoring tools:
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Airline \taubench (hardest for LLM judges)}: Even Claude
    Sonnet~4 reaches only 40\% DR. Only Galea achieves both high DR (100\%)
    and low FPR (0\%) here.
  \item \textbf{ATBench (middle ground; partially structural, partially
    semantic)}: Galea achieves 52.5\% DR with 0\% FPR---some safety patterns
    co-occur with structural anomalies that heuristics can detect.
    Claude Sonnet~4 achieves 100\% DR and Llama~4~Scout achieves 70\% DR,
    confirming that semantic understanding substantially outperforms structural
    analysis but is not uniquely necessary. Rule-driven platforms (LangSmith,
    Braintrust) achieve 0\% DR, confirming that rules written for
    process/structural failures do not generalize to semantic safety.
  \item \textbf{Toolathlon (hardest for all systems except Claude Sonnet~4)}:
    Long-horizon multi-tool failures involve no structural signal (no error
    strings, no loops, no step-count anomalies that are not normal for
    600+-tool environments). Every system except Claude Sonnet~4 achieves
    0\% DR. Claude Sonnet~4 achieves 100\% DR with 67\% FPR: it can detect
    genuine long-horizon failures but over-flags passing trajectories,
    suggesting calibration challenges in extremely long contexts. Llama~4~Scout
    reaches 14.3\% DR with 0\% FPR, indicating that open-weight models can
    occasionally identify long-horizon failures but lack the consistent
    semantic depth of frontier models.
\end{itemize}

This six-tier spectrum has a critical practical implication: benchmarking
monitoring tools on any single domain gives a highly misleading picture of
production performance. The complete matrix---spanning structural-easy
(\swebench) through long-horizon-hard (Toolathlon)---is necessary to
characterize operational coverage across the full failure landscape.
Notably, Claude Sonnet~4 is the only system achieving any non-zero DR
across all six tiers, and no single system achieves high DR \emph{and} low
FPR uniformly.
...
\subsection{ATBench: Semantic Safety as the Inverse Domain}
\label{sec:analysis:atbench}

ATBench~\citep{ai45research2025atbench} provides an important counterpoint to
the structural vs.\ semantic binary. Its 1,000 trajectories (503 safe, 497
unsafe) span 14 failure modes and 8 risk sources. We evaluate on a
proportionally sampled 200-trajectory subset ($\approx$101 safe, $\approx$99
unsafe). Failure modes are defined at the semantic level: tool description
injection, harmful action execution, data leakage, and related safety
violations.

The complete ATBench results reveal a more nuanced picture than the original
pilot suggested. Claude Sonnet~4 achieves 100\% DR [98.1\%, 100.0\%] on the
200-trajectory evaluation, confirming that frontier models can reliably
identify semantic safety failures. Llama~4~Scout achieves 70\% DR on ATBench,
indicating that open-weight models can detect a substantial majority of safety
failures---a meaningful improvement over their performance on \taubench domains.
Most significantly, Galea achieves 52.5\% DR [45.5\%, 59.5\%] on ATBench with
0\% FPR: this is Galea's \emph{middle-ground} result, between its 100\% on
\taubench and 0\% on Toolathlon. Some ATBench safety patterns are structurally
co-detectable (tool-description anomalies that deviate from normal schema
patterns, unusual tool-call sequences preceding harmful outcomes), making
partial structural detection possible.

Rule-driven systems (LangSmith, Braintrust) achieve 0\% DR on ATBench,
confirming that rules written for process and structural failures do not
transfer to semantic safety.

\paragraph{ATBench in the difficulty spectrum.}
ATBench occupies a fifth tier in the domain difficulty spectrum---harder than
airline for heuristic systems (0\% for naive/rule-based; 52.5\% for Galea)
but easier than Toolathlon for LLM judges (70--100\% vs.\ 0--14\% on
Toolathlon for open-weight models). Together with Toolathlon, ATBench
completes the spectrum from structurally trivial (\swebench) through
semantically hard (ATBench) to long-horizon impossible (Toolathlon).

\paragraph{Ensemble implication.}
The ATBench results strengthen the ensemble case: a Galea+Claude Sonnet~4
architecture would combine Galea's 100\% DR on \taubench retail/airline (where
Claude reaches only 40--100\%) with Galea's 52.5\% structural coverage on
ATBench and Claude's 100\% semantic coverage. Neither system alone covers the
full six-domain matrix; together they would achieve comprehensive coverage
across structural, semantic safety, and (partially) long-horizon failure
classes---with Toolathlon remaining the hardest open problem.

\subsection{Toolathlon: Long-Horizon Tool Use as the Hardest Domain}
\label{sec:analysis:toolathlon}

Toolathlon~\citep{toolathlon2026} (HKUST-NLP, ICLR 2026) provides the
most demanding evaluation in the \atfd benchmark. Its 100 trajectories---drawn
from Claude~4~Sonnet runs on long-horizon multi-tool tasks with 600+
available tools---represent the frontier of agent complexity: individual
runs may contain up to 16,000 messages, and task completion requires
coordinating across dozens of sequential tool calls where intermediate
outputs feed downstream steps.

The results establish Toolathlon as categorically different from all other
evaluated domains. Every system except Claude Sonnet~4 achieves exactly
0\% DR: the naive heuristic, Galea, LangSmith, Braintrust, and
Llama~4~Scout all emit no valid findings on Toolathlon failure trajectories.
This is not a consequence of noise or sample size---it reflects a
fundamental mismatch between available detection signals and long-horizon
failure modes. A trajectory that selects the wrong tool 47 steps into a
100-step plan, triggering a silent but incorrect intermediate state that
propagates to a wrong final answer, leaves no structural fingerprint that
pattern matching can exploit. There is no error string, no loop, and no
step-count anomaly distinguishable from normal task complexity.

Claude Sonnet~4 achieves 100\% DR on Toolathlon (n=100), confirming that
frontier semantic understanding can identify these failures. However, its
67\% FPR is the highest of any Claude cell across all six domains. This
FPR pattern suggests that in extremely long contexts, even frontier models
struggle to distinguish a genuinely failed plan from a passing one with
unusual intermediate states. Llama~4~Scout achieves 14.3\% DR with 0\%
FPR---occasional detection with no false alarms---suggesting that open-weight
models can recognize the most egregious long-horizon failures but lack the
consistent depth needed for high recall.

\paragraph{Why Toolathlon differs from ATBench.}
Both Toolathlon and ATBench are hard for heuristic systems, but for
fundamentally different reasons. ATBench failures are \emph{semantically
obvious} to language models (harmful instructions are recognizable to any
sufficiently capable LLM) but \emph{structurally invisible}. Toolathlon
failures are hard for \emph{all} system classes: they require not only
semantic understanding of the task goal but also the ability to trace
causality across a very long context window to identify where in a
multi-step plan the failure originated. This makes Toolathlon the most
demanding domain for current monitoring technology and the one where
future improvements---in context length, multi-step reasoning, and
long-horizon plan evaluation---are most needed.

\paragraph{Ensemble implication.}
The Toolathlon results narrow the potential coverage of any ensemble
that combines current systems. A Galea+Claude Sonnet~4 ensemble would
achieve near-complete coverage of the first five tiers (structural,
synthetic, retail, airline, ATBench), but on Toolathlon it would rely
entirely on Claude Sonnet~4---with its 67\% FPR overhead. Reducing
FPR on long-horizon trajectories is the most pressing open problem
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Dataset scale and coverage gaps.} While the dataset
    comprises 2,462 trajectories, LLM judge evaluation was limited to
    20--200 trajectories for most domains due to API throughput constraints.
    The \swebench slice (100 trajectories) was evaluated at full scale only
    for heuristic systems; Claude Sonnet~4 was evaluated on 20-trajectory
    subsets for retail, airline, and \swebench, but at full scale for ATBench
    (200) and Toolathlon (100). Some subcategories (\eg
    \texttt{permission\_escalation}) have only 5 examples, limiting
    statistical power---as reflected in the wide confidence intervals for
    small-N cells.

  \item \textbf{Domain coverage.} The current six domains (customer service,
    coding, synthetic, agent safety, and long-horizon tool use) provide a
    broad foundation across structural, semantic, and multi-step failure
    modes, but do not cover all deployment contexts (\eg medical, financial,
    legal domains where monitoring is most critical).
...
scorers---across all 6 domains on 2,462 trajectories from \taubench,
\swebench, synthetic sources, ATBench, and Toolathlon reveals a striking
\emph{complementarity} in detection profiles, an equally stark
\emph{configuration effort gap}, a pervasive \emph{domain calibration}
failure mode, and a clear six-level \emph{domain difficulty spectrum}
from structurally obvious (\swebench) through long-horizon-hard (Toolathlon).
...
plateaus at 78\% on synthetic failures requiring semantic analysis, reaches
52.5\% DR on ATBench (a middle ground reflecting partial structural
detectability of safety patterns), and achieves exactly 0\% DR on Toolathlon
where multi-step plan failures leave no structural footprint.
Open-weight LLM judges achieve 100\% DR on synthetic failures but
struggle on real trajectories (Llama~4~Scout: 25\% on retail, 20\% on
airline, 70\% on ATBench, 14\% on Toolathlon).
Claude Sonnet~4 is the \textbf{only system achieving non-zero DR across all
six domains}: it achieves 100\% DR across synthetic, retail, \swebench,
ATBench, and Toolathlon, with FPR escalating from 0\% (\swebench) to 6\%
(retail) to 20\% (airline) to 67\% (Toolathlon). Its airline performance
(40\% DR) is the one domain where even Claude Sonnet~4 underperforms Galea's
structural approach (100\% DR, 0\% FPR). The highest category alignment
(CA = 0.489 on synthetic) remains Claude Sonnet~4's.

Toolathlon establishes a new frontier: every system except Claude Sonnet~4
achieves 0\% DR on long-horizon multi-tool trajectories. This is the hardest
domain for all currently evaluated systems and the one where future
improvements in long-context reasoning will have the greatest impact on
monitoring coverage. ATBench reveals a middle ground for Galea (52.5\% DR),
confirming that some safety failure patterns are structurally co-detectable
even when others require full semantic understanding.
...
FPR on \taubench domains, 100\% DR with 100\% FPR on \swebench, and 0\% DR
on ATBench and Toolathlon---making their event-count threshold operationally
unusable on coding trajectories without domain recalibration and completely
ineffective on semantic safety and long-horizon domains.
...
These findings establish six conclusions about agent failure detection.
First, it is a genuinely difficult problem: no system achieves high DR and
low FPR simultaneously across all six domains, and even frontier models
...
semantic failures on synthetic and safety domains; Claude Sonnet~4 bridges
all six domains but with escalating FPR on harder ones.
...
Fifth, the domain difficulty spectrum is six-tiered: \swebench (easy,
structural signals), synthetic (moderate, injected patterns), retail
(hard, subtle state errors), airline (hardest for LLMs, domain-specific
policy semantics), ATBench (middle ground, partially structural + partially
semantic safety), and Toolathlon (hardest for all except Claude Sonnet~4,
multi-step plan failures with no structural signal).
Sixth, \textbf{Toolathlon is the hardest domain for all systems except
Claude Sonnet~4}: the gap between structural/rule/open-weight approaches
(all 0\% DR) and frontier semantic understanding (100\% DR) is total, making
long-horizon tool use the most pressing open problem for agent monitoring.
Benchmarking monitoring tools on any single domain would give a misleading
picture of production utility; the complete six-domain matrix is necessary
to characterize operational coverage.

\paragraph{Future work.}
This release provides the complete 7-system $\times$ 6-domain evaluation on
2,462 trajectories. Immediate next steps include:
...
(2)~completing Llama~3.3~70B evaluation on retail, airline, \swebench,
ATBench, and Toolathlon domains (currently blocked by Groq free-tier rate
limits);
(3)~scaling Claude Sonnet~4 and Llama~4~Scout evaluation on all six domains
to full dataset sizes to tighten confidence intervals;
(4)~evaluating the Galea full-stack system (with LLM-augmented analysis)
beyond the heuristic-only configuration reported here, particularly on
ATBench and Toolathlon where semantic gaps remain;
(5)~running the multi-judge consensus pipeline for ground truth validation;
(6)~applying domain-specific quality rubrics to enable the \texttt{degraded}
tier and QDR measurement;
(7)~expanding domain coverage to medical, legal, and financial workflows;
(8)~adding streaming evaluation for real-time monitoring tools;
(9)~investigating why category alignment is so low and whether alternative
prompting strategies or fine-tuned models can improve failure type
identification;
(10)~formally evaluating heuristic+LLM ensemble configurations---in
particular, measuring whether a Galea+Claude Sonnet~4 ensemble achieves
near-complete coverage across all six domains; and
(11)~investigating prompt calibration strategies to reduce Claude Sonnet~4's
67\% FPR on Toolathlon while maintaining its 100\% DR, which is the most
pressing precision challenge in long-horizon monitoring.
